...
@Component({
  ...
  templateUrl: './dinero-valores.component.html',
...
export class DineroValoresComponente {

  @Input() listaDineroValores: any = [];
  ...
  constructor(public general: CotizacionRamoGeneral, public sesion: SesionService, public validacion: ValidacionPipe,
    ...
    this.general.asignacionProvinciaSubramos(this.listaDineroValores);
    setTimeout(() => {
      this.general.asignacionProvinciaSubramos(this.listaDineroValores);
      this.obtenerUnionDeducibles();

      console.log(this.listaDineroValores);
    }, 1000);
  }

  public gestionTasaDineroValores(subramo: any, tasa: any) {
    this.general.asignacionNuevoValorTasa(tasa, this.listaDineroValores, subramo);
    this.general.calcularPrimaSubramo(this.listaDineroValores, subramo);
  }

  public gestionRamoDineroValores(valor: any, subramo: any, ubicacion: any) {
    this.validacion.gestionReglasRamosSubRamos(this.listaDineroValores, subramo, ubicacion, valor, this.lstRamos, this.general, this.reglasIndividual, this.reglasGlobal, this.reglasVertical, this.reglasGrupal);
    this.general.calcularTasaProvincias(this.listaDineroValores, subramo);
    this.general.calcularPrimaSubramo(this.listaDineroValores, subramo);
  }
  ...
  public obtenerUnionDeducibles() {
    var deducibles = [];
    this.deducibles = [];
    for (let subramos of this.listaDineroValores) {
